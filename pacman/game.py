...
movement (Red = left, Green = right), Player 2 owns vertical movement
(Blue = up, Yellow = down) -- so neither can cross the maze alone without the
other's help. Each player's pair sits physically next to each other on the
panel (Red+Green on the left, Blue+Yellow on the right) so the two of them
aren't reaching across each other's hands. White is shared off to the side
to cash in a stored power pellet and scare the ghosts for a while. Clear
every pellet to win; get caught by a ghost while not scared and the whole
team loses.
...
class PacDuoState:
    ...
    def next_state(self, input: Input) -> State | None:
        ...
        if current_time - self.last_move_time >= MOVE_INTERVAL_MS:
            ...
            # Player 1: Red = left, Green = right. Player 2: Blue = up, Yellow = down.
